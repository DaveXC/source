...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Maybe try even harder?', 0.4], //From 20% to 39%
		['Worse than average', 0.5], //From 40% to 49%
		['Average', 0.6], //From 50% to 59%
		['Ok', 0.69], //From 60% to 68%
		['Nice', 0.7], //69%
		['Good', 0.8], //From 70% to 79%
		['Jammin', 0.9], //From 80% to 89%
		['Radical!', 1], //From 90% to 99%
	...
	override public function create()
	{
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 19, 400, "", 8);
		...
		scoreTxt = new FlxText(0, 36, FlxG.width, "", 32);
		...
		switch (SONG.song.toLowerCase())
		{
			case 'opposition-fucked':
				credits = "Fuck you! You are finished!";
			default:
				credits = '';
		}
		...
		versionTxt = new FlxText(0, FlxG.height - 24, 0, " DaveXC Engine 1.0 (PE " + MainMenuState.psychEngineVersion + ")");
		...
		if (creditsText)
		{
			var creditsWatermark = new FlxText(4, 50, 0, credits, 16);
	     	creditsWatermark.setFormat(Paths.font("pixel.otf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		chartingTxt = new FlxText(400, 55, FlxG.width - 800, "CHARTING MODE", 32);
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			var daCredits:CreditPopup = new CreditPopup(-1280, 150, SONG.song);
			daCredits.camera = camHUD;
			daCredits.x = daCredits.width * -1;
			add(daCredits);
			FlxTween.tween(daCredits, {x: 0}, Conductor.crochet / 2000, {
				ease: FlxEase.backOut,
				startDelay: 1,
				onComplete: function(twn:FlxTween)
				{
					FlxTween.tween(daCredits, {x: -daCredits.width}, 1, {
						ease: FlxEase.backOut,
						startDelay: 1.75,
						onComplete: function(twn:FlxTween)
						{
...
iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - (iconP2.width - iconOffset);
	...
	else if (healthBar.percent > 80)
	{
		...
		scoreTxt.color = CoolUtil.smoothColorChange(scoreTxt.color, FlxColor.CYAN, 0.3);
	...
	if(SONG.song.toLowerCase() == "opposition-fucked")
		{
		...
		else
		{
			...
			if (!paused)
			{
				...
				if(updateTime) {
					...
					timeTxt.text = SONG.song + " - " + CoolUtil.difficultyString() + ' (' + minutesRemaining + ':' + secondsRemaining + ')';
					if(SONG.song.toLowerCase() == "opposition-fucked")
						{
	...
	override function beatHit()
	{
		...
		if(curBeat % cameraBeatSpeed == 0)
		{
		    ...
		    iconP1.setGraphicSize(Std.int(iconP1.width + 80));
            iconP2.setGraphicSize(Std.int(iconP2.width + 80));
