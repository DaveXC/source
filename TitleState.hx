...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('fnf'));
		add(ngSpr);
		ngSpr.visible = false;
	...
	function createCoolText(textArray:Array<String>, ?offset:Float = 0)
		{
			for (i in 0...textArray.length)
			{
				...
				money.setFormat("_sans", 48, FlxColor.WHITE, CENTER);
		...
		function addMoreText(text:String, ?offset:Float = 0)
		{
			...
			coolText.setFormat("_sans", 48, FlxColor.WHITE, CENTER);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					#if PSYCH_WATERMARKS
					createCoolText(['Psych Engine Modified by'], 15);
					#else
				...
				case 5:
					#if PSYCH_WATERMARKS
					createCoolText(['A fan modification', 'of'], -40);
					#end
				case 7:
		...
		if (!skippedIntro)
		{
			...
			else //Default! Edit this one!!
			{
			 ...
			 FlxTween.tween(gradientBar, {'scale.y': 1.3}, 4, {ease: FlxEase.quadInOut});
				...
				gfDance.y = 0;
				FlxTween.tween(gfDance,{y: -20}, 0.4, {ease: FlxEase.expoInOut});
