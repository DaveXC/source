...
class FreeplayState extends MusicBeatState
{
	...
	private var AllPossibleSongs:Array<String> = ["base", "joke"];
	...
	public function LoadProperPack()
		{
			switch (AllPossibleSongs[CurrentPack].toLowerCase())
			{
				...
				case 'joke':
					addWeek(['Lacuna'], 1, ['pbambi']);
					addWeek(['Opposition-Fucked'], 2,['pearl']);
			}
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.BACK)
			...
			if (accepted)
			{
				...
				{
					FlxTween.tween(FlxG.camera, {zoom: 8}, 0.8, {ease: FlxEase.expoIn});
		...
		if (fuckyou)
		{
			...
			PlayState.SONG = Song.loadFromJson("opposition-fucked-hard", "opposition-fucked"); // you dun fucked up again
	...
	{
		MusicBeatState.switchState(new ResetScoreSubState(songs[curSelected].songName, curDifficulty, songs[curSelected].songCharacter));
		FlxG.sound.play(Paths.sound('scrollMenu'));
	}
	super.update(elapsed);
}
